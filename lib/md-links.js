...
mdLinks.mdLinks = (newPath, options) => {
  return new Promise((resolve, reject) => {
    ...
    if (validateFile === '.md') {
      mdLinks.readFile(pathAbsolute).then((data) => {
        resolve(data);
      });
    }        
  });
...
mdLinks.readFile = (newPath) => {
  return new Promise((resolve, reject) => {
    fs.readFile(newPath, 'utf8', (err, data) => {
      if (err) reject(err);
      data = data.split('\n').map(element => mdLinks.markdownLinkExtractor(newPath, element, data.indexOf(element) + 1)).filter(element => element.length !== 0).reduce((value1, value2) => value1.concat(value2));
      resolve(data);
    });
  });
};
